...
<script type="text/javascript">
    ...
    jQuery(function($) {
        ...
        jQuery(grid_selector).jqGrid({
            datatype: "json",
            mtype: "post",
            url: "<c:url value='/json/zteMovie_list.do'/>",
            postData: {
                code: $("#code").val(),
                name: $("#name").val(),
            ...
            height: 560,
            colNames:['ID','program.bestvStatus',
                ...
                '<fmt:message key="zte.content.actorDisplay"/>',
                '<fmt:message key="zte.content.writerDisplay"/>',
                '<fmt:message key="zte.content.originalCountry"/>',
                '<fmt:message key="zte.content.language"/>',
                ...
                '<fmt:message key="zte.content.releaseYear"/>',
                '<fmt:message key="zte.content.orgAirDate"/>',
                '<fmt:message key="zte.content.createTime"/>',
                '<fmt:message key="zte.content.updateTime"/>'
            ],
            colModel:[
                {name:'program.id',index:'id', width : 100, hidden:true},
                {name:'program.bestvStatus',index:'bestv_status', width : 100, hidden:true},
                {name : 'program.code', index : 'code_', width : 120, align:'center', sortable : false},
                {name : 'program.action', index : 'action_', width : 120, align:'center', sortable : false},
                {name : 'program.name', index : 'name_', width : 250, align:'center', sortable : false},
                {name : 'program.originalName', index : 'original_name', width : 250, align:'center', sortable : false},
                {name : 'program.orderNumber', index : 'order_number', width : 120, align:'center', sortable : false},
                {name : 'program.actorDisplay', index : 'actor_display', width : 200, align:'center', sortable : false},
                {name : 'program.writerDisplay', index : 'writer_display', width : 150, align:'center', sortable : false},
                {name : 'program.originalCountry', index : 'original_country', width : 100, align:'center', sortable : false},
                {name : 'program.language', index : 'language_', width : 100, align:'center', sortable : false},
                {name : 'program.statusStr', index : 'status_', width : 100, align:'center', sortable : false},
                {name : 'program.bestvStatusStr', index : 'bestv_status', width : 100, align:'center', sortable : false},
                {name : 'program.type', index : 'type_', width : 120, align:'center', sortable : false},
                {name : 'category.name', index : 'category_name', width : 200, align:'center', sortable : false},
                {name : 'program.releaseYear', index : 'release_year', width : 100, align:'center', sortable : false},
                {name : 'program.orgAirDate', index : 'org_air_date', width : 100, align:'center', sortable : false},
                {name : 'program.createTime', index : 'create_time', width : 200, align:'center', sortable : true, formatter:"date", formatoptions: {srcformat:'Y-m-d H:i:s',newformat:'Y-m-d H:i:s'}},
                {name : 'program.updateTime', index : 'update_time', width : 200, align:'center', sortable : true, formatter:"date", formatoptions: {srcformat:'Y-m-d H:i:s',newformat:'Y-m-d H:i:s'}}
            ],
            shrinkToFit : false,
            hidegrid : false,
            viewrecords : true,
            ...
            caption: '<fmt:message key="zte.movie.list" />',
            <cas:havePerm url="/pages/contentShow/zteMovieDetail.jsp">
                ondblClickRow : function (rowid, iRow, iCol, e) {
                    var code = $(grid_selector).getRowData(rowid)["program.code"];
                    openMainPage("/pages/contentShow/zteProgramDetail.jsp", {"code": code}, function () {
                    });
                },
            </cas:havePerm>
            toolbar: [true,'top'],
        ...
        function onOrOffLine(){
            ...
            for (var index in ids){
                ...
                codes.push(rowData["program.code"]);
            }
            ...
            $.post('<c:url value="/json/zteMovie_onOrOffLine.do"/>', {"codes": codeStr.substring(0, codeStr.length-1), "actionType": actionType}, function (result) {
                bootbox.alert("操作成功！");
